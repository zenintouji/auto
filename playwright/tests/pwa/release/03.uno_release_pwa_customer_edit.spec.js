...
test('test', async ({ page }) => {
  ...
  await expect(page.getByText('김정연(').nth(1)).toBeVisible(); // 소개자
  await expect(page.getByText('BEST')).toBeVisible(); // 고객등급
  await expect(page.getByText('-', { exact: true }).nth(9)).toBeVisible(); // 신환 구분일
  ...
  await expect(page.getByText('최지안')).toBeVisible(); // 담당상담사
  await expect(page.getByText('강슬아').nth(1)).toBeVisible(); // 담당의사
  await expect(page.getByText('리프팅, 성형, 피부 시술')).toBeVisible(); // 관심항목
  await expect(page.getByText('SNS, 굿닥, 병원홈페이지')).toBeVisible(); // 최초내원경로
  await expect(page.getByText('-', { exact: true }).nth(10)).toBeVisible(); // 주소
  await expect(page.getByText('상세주소 자동화')).toBeVisible(); // 상세주소 자동화
  await expect(page.getByText('전문특수직')).toBeVisible(); // 회사원
  ...
  await expect(page.locator('span').filter({ hasText: '김정연/1994-07-07/남성/' })).toBeVisible(); // 가족관계
  ...
  await page.getByRole('combobox', { name: '소개자를 검색하세요' }).fill('김정연45');
  await expect(page.getByText('-8888-9997')).toBeVisible();
  await page.getByText('-8888-9997').click();
  // 메모
  ...
  await page.getByRole('option', { name: '디자인' }).click();
  // 결혼여부
  ...
  await page.getByRole('combobox', { name: '가족관계를 등록하세요' }).fill('김정연32');
  await page.waitForTimeout(3000);
  await expect(page.getByText('-3322-3322')).toBeVisible();
  await page.getByText('-3322-3322').click();
  // 가족관계 추가
