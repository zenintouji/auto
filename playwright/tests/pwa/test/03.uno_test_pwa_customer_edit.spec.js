...
test('test', async ({ page }) => {
  ...
  await expect(page.getByText('김정연(20250110_80932)')).toBeVisible(); // 소개자
  ...
  await expect(page.getByText('병원홈페이지, 굿닥, SNS')).toBeVisible(); // 최초내원경로
  ...
  await expect(page.getByText('-9898-9898')).toBeVisible();
  await page.getByText('-9898-9898').click();
  // 메모
  ...
  await page.waitForTimeout(3000);
  await expect(page.getByText('-8989-8989')).toBeVisible();
