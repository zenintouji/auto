...
test('test', async ({ page }) => {
  ...
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '의사' })).toBeVisible();
  ...
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '병원장' })).toBeVisible();
  ...
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '우노병원' })).toBeVisible();
  await page.getByRole('option', { name: '우노병원' }).click();
  // 저장
