...
test('test', async ({ page }) => {
  ...
  await page.getByText('제휴회사굿닥바비톡').hover();
  ...
  await page.getByText('김정연', { exact: true }).click();
  // 메모 수정
  ...
  await page.getByText('보톡스이관리프팅').hover();
  await page.getByRole('button', { name: 'Clear' }).click();
  await expect(page.getByText('관심항목 (최대 3개)')).toBeVisible();
  ...
  await expect(page.getByText('김정연45')).toBeVisible();
  await page.getByText('김정연45').click();
  // 진료정보 > 신체정보
