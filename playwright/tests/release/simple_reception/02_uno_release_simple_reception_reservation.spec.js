...
test('test', async ({ page }) => { 
  ...
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '전화상담' })).toBeVisible();
  ...
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '보톡스', exact: true })).toBeVisible();
  ...
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '필러' })).toBeVisible();
