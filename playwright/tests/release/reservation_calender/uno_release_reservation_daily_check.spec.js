...
test('Reservation > Daily Check', async ({ page }) => {
  ...
  const statusBoard = new StatusBoard(page);
  ...
  await reservation.changeDateToToday();
  ...
  // 현황판 이동
  await statusBoard.moveToStatusBoard();
  await statusBoard.checkCustomerOnDay();
  await statusBoard.reEnterInChart();
  await reservation.enterReservation();
  
  await reservation.selectEdit(); // 수정 진입
  await reservation.changeDateToYesterday(); // 어제 날짜로 수정
  ...
  // 어제 날짜 이동해서 확인
  await statusBoard.moveToStatusBoard();
  await statusBoard.moveToYesterday();
  await statusBoard.checkCustomerOnDay();
  ...
  await reservation.changeDatetoTomorrow();
  await reservation.selectEditCompleteButton();
  await reservation.checkEditSuccessText();
  ...
  // 내일 날짜 이동해서 확인
  await statusBoard.moveToStatusBoard();
  await statusBoard.moveToTomorrow();
  await statusBoard.moveToTomorrow();
  await statusBoard.checkCustomerOnDay();
