...
test('test', async ({ page }) => {
  ...
  await page.getByRole('option', { name: '31zz' }).click();
  // 투약량
  ...
  // 수술간호 저장 확인
  await expect(page.getByRole('cell', { name: '수술간호내용', exact: true })).toBeVisible();
  await expect(page.getByRole('cell', { name: '수술간호내용 입력 자동화' })).toBeVisible();
  await expect(page.getByText('통합차트')).toBeVisible();
  // 통합차트 닫기
  await page.locator('div').filter({ hasText: /^3D Meta-Vu$/ }).getByRole('button').nth(2).click();
  await expect(page.getByText('고객조회 내역')).toBeVisible();
  await expect(page.getByRole('button', { name: '수술간호 조회' })).toBeVisible();
  await page.getByRole('button', { name: '수술간호 조회' }).click();
  // 수술간호 조회
  await expect(page.getByRole('paragraph').filter({ hasText: '수술간호 조회' })).toBeVisible();
  // 고객명 검색
  ...
  const selectBirth_From = page.locator('[role="combobox"]').nth(8);
  await selectBirth_From.click();
  await page.getByRole('option', { name: '1994년' }).click();
  const selectBirth_To = page.locator('[role="combobox"]').nth(9);
  ...
  const selectInteresting = page.locator('[role="combobox"]').nth(13);
