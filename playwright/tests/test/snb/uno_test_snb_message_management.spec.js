...
test('test', async ({ page }) => {
  ...
  await expect(page.getByRole('cell', { name: '#{병원명} 사랑하는 #{고객명} 고객님 예약 안내드립니다^^ ■ 예약 일자 : #{예약일} ■ 예약 시간 : #{예약시간} [필독] 내원 시 참고해 주세요 ^^ ♥ 코로나19 예방을 위해 마스크 착용은 필수인 점 양해 부탁드립니다. ♥ 전 지점 위생에 신경쓰며 지점 내 손소독제가 비치 되어 있습니다. ♥ 최근 3주 이내 해외 방문하신 경우 예약의 제한이 있을 수 있습니다#{고객명}#{문진URL}', exact: true })).toBeVisible();
  ...
  // .click();
  await expect(page.getByRole('button', { name: '사용', exact: true }).nth(1)).toBeVisible();
  await page.getByRole('button', { name: '사용', exact: true }).nth(1).click();
  ...
  await expect(page.getByRole('row', { name: '퇴원 전체 즉시발송 #{병원명} 사랑하는 #{고객명} 고객님 예약 안내드립니다^^ ■ 예약 일자 : #{예약일} ■ 예약 시간 : #{예약시간} [필독] 내원 시 참고해 주세요 ^^ ♥ 코로나19 예방을 위해 마스크 착용은 필수인 점 양해 부탁드립니다. ♥ 전 지점 위생에 신경쓰며 지점 내 손소독제가 비치 되어 있습니다. ♥ 최근 3주 이내 해외 방문하신 경우 예약의 제한이 있을 수 있습니다#{고객명}#{문진URL} 문진 자동화 (070) 5167-8646 이미지가 첨부된 경우에는 알림톡 템플릿 심사 요청을 하지 않습니다. 미사용', exact: true }).getByRole('button').first()).toBeVisible();
  await page.getByRole('row', { name: '퇴원 전체 즉시발송 #{병원명} 사랑하는 #{고객명} 고객님 예약 안내드립니다^^ ■ 예약 일자 : #{예약일} ■ 예약 시간 : #{예약시간} [필독] 내원 시 참고해 주세요 ^^ ♥ 코로나19 예방을 위해 마스크 착용은 필수인 점 양해 부탁드립니다. ♥ 전 지점 위생에 신경쓰며 지점 내 손소독제가 비치 되어 있습니다. ♥ 최근 3주 이내 해외 방문하신 경우 예약의 제한이 있을 수 있습니다#{고객명}#{문진URL} 문진 자동화 (070) 5167-8646 이미지가 첨부된 경우에는 알림톡 템플릿 심사 요청을 하지 않습니다. 미사용', exact: true }).getByRole('button').first().click();
  await expect(page.getByRole('heading', { name: '자동전송 입력 close' })).toBeVisible();
  // 자동전송 수정 > 발신번호
  await expect(page.getByRole('combobox', { name: '발신번호를 선택하세요' })).toBeVisible();
  await page.getByRole('combobox', { name: '발신번호를 선택하세요' }).click();
  ...
  await expect(page.getByRole('cell', { name: '#{병원명} ○#{고객명}사랑하는 #{고객명} 고객님 예약 안내드립니다^^ ■ 예약 일자 : #{예약일} ■ 예약 시간 : #{예약시간} [필독' })).toBeVisible();
  // 자동전송 미사용
  ...
  await expect(page.getByRole('row', { name: '선택 test test img' }).getByRole('button')).toBeVisible();
  await page.getByRole('row', { name: '선택 test test img' }).getByRole('button').click();
  // 주기별 자동전송 입력 > 변환문구
  await expect(page.getByText('변환문구')).toBeVisible();
  await page.getByText('변환문구').click();
  ...
  await expect(page.getByRole('cell', { name: '발신번호' }).nth(0)).toBeVisible();
