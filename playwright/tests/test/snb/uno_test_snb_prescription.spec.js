...
test('test', async ({ page }) => {
  ...
  // 묶음처방 등록
  await expect(page.getByRole('row', { name: '등록 1 실리프팅 후 처방 세파록스캡슐(세파클러수화물)_(0.25g/1캡슐)' }).getByRole('button')).toBeVisible();
  await page.getByRole('row', { name: '등록 1 실리프팅 후 처방 세파록스캡슐(세파클러수화물)_(0.25g/1캡슐)' }).getByRole('button').click();
  // 처방전 > 용법 입력
  ...
  await expect(page.getByRole('cell', { name: 'dev' }).nth(2)).toBeVisible();
  // 교부번호
  ...
  await expect(page.getByRole('cell', { name: 'dev' }).nth(3)).toBeVisible();
  // 상태
