...
test('test', async ({ page }) => {
  ...
  await expect(page1.getByRole('cell', { name: 'dev' }).nth(0)).toBeVisible();
  ...
  await expect(page1.getByRole('row', { name: '등록 1 실리프팅 후 처방 세파록스캡슐(세파클러수화물)_(0.25g/1캡슐)' }).getByRole('cell').nth(1)).toBeVisible();
  await page1.getByRole('row', { name: '등록 1 실리프팅 후 처방 세파록스캡슐(세파클러수화물)_(0.25g/1캡슐)' }).getByRole('cell').nth(1).click();
